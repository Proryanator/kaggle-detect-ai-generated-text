...
train = pd.read_csv("../data/competition/train_essays.csv")
test = pd.read_csv('../data/ensemble-learning/test_essays.csv')

# clears out any nans with 0's
train = train.fillna(0)
test = test.fillna(0)

train_ = train[train.generated == False].reset_index(drop=True)
train_ = train[train["label"] == 1].sample(8000)

train = train[train.generated == True].reset_index(drop=True)
